...
class CSC_DataSet(Dataset):
    ...
    def __getitem__(self, item):
        def compare_tokens(input_token, output_token):
            labels = []
            for char_in, char_out in zip(input_token, output_token):
                if char_in == char_out:
                    labels.append(0)
                else:
                    labels.append(1)
        ...
        output_token = data['Output'].strip()
        ...
        label=compare_tokens(input_token, output_token)
        label = [0] + label + [0]
